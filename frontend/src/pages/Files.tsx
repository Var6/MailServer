...
function FileIcon({ contentType, isDir }: { contentType?: string; isDir?: boolean }) {
  ...
  if (t.includes("spreadsheet") || t.includes("excel")) return <FileSpreadsheet size={28} className="text-green-600" />;
  if (t.includes("presentation") || t.includes("powerpoint")) return <Presentation size={28} className="text-orange-500" />;
  ...
  if (t.includes("text/") || t.includes("json") || t.includes("xml")) return <FileCode size={28} className="text-blue-400" />;
    ...
    <div className="h-full bg-white flex flex-col" {...getRootProps()}>
      ...
      <div className="px-6 py-3 border-b border-gray-100 flex items-center justify-between flex-shrink-0">
        ...
        <div className="flex items-center gap-2">
          <button
            onClick={open}
            className="flex items-center gap-1.5 text-sm text-blue-600 hover:text-blue-700 border border-blue-200
                       hover:border-blue-400 rounded-full px-3 py-1.5 transition-colors"
          >
          ...
          <a
            ...
            className="flex items-center gap-1.5 text-sm text-[#5f6368] hover:text-[#202124] border border-gray-200
                       hover:border-gray-300 rounded-full px-3 py-1.5 transition-colors"
          >
        ...
        {!isLoading && data && data.length > 0 && (
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-2">
            {(data as any[]).map((file: any, i: number) => (
              <div
                key={i}
                className="flex flex-col items-center p-4 rounded-xl hover:bg-gray-50 cursor-pointer
                           group transition-colors border border-transparent hover:border-gray-200"
                onClick={() => file.isDirectory && navigate(file.name)}
              >
                <div className="mb-2">
                  <FileIcon contentType={file.contentType} isDir={file.isDirectory} />
                </div>
